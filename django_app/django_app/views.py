...
def train_ddpg(request):
    if request.method == 'POST':
        ...
        # Prepare and return the response
            response_data = {
            'ddpg_train_data_log': ddpg_train_data_log, 
...
def evaluate_ddpg(request):
    if request.method == 'POST':
        try:
            ...
            # Prepare and return the response
            return JsonResponse({
                'ddpg_eval_data_log': ddpg_eval_data_log,
            })
        except Exception as e:
            return JsonResponse({'error': str(e)})
    else:
...
def train_ppo(request):
    if request.method == 'POST':
        try:
            ...
            response_data = {
            'ppo_train_data_log': ppo_train_data_log,
...
def evaluate_ppo(request):
    if request.method == 'POST':
        try:
            ...
            # Prepare and return the response
            return JsonResponse({'ppo_eval_data_log': ppo_eval_data_log})
        except Exception as e:
            return JsonResponse({'error': str(e)})
    else:
        return JsonResponse({'error': 'Only POST requests are allowed'})
    ...
    if request.method == 'POST':
        try:
            ...
            strategy_action, ddpg_action, ppo_action = perform_inference(
                ...
                ddpg_agent_path=ddpg_agent_path, ppo_agent_path=ppo_agent_path
                )
            response_data = {
