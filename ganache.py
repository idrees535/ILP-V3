...
# Port in ganache_url must match ganache call
port_number = "8545"
ganache_url = confFileValue("general", "GANACHE_URL")
assert port_number in ganache_url

# get ganache going, populating accounts found in conf file
...
# launch ganache and give each private account 100 eth.
amount_eth = 100000
amount_wei = eth_utils.to_wei(amount_eth, "ether")

ganache_command = f'ganache --port {port_number} --gasLimit 10000000000 --gasPrice 1 ---hardfork istanbul --account="{alice_private_key},{amount_wei}" --account="{bob_private_key},{amount_wei}" --account="{factory_deployer_private_key},{amount_wei}"'  # pylint: disable=line-too-long
