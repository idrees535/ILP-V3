...
all_agents = ['ddpg_1','ddpg_tempest_1000x20','ddpg_tempest_2000x50']
all_agents =['ppo_tempest_2000x15']
for ddpg_agent_name in all_agents:
    ...
    def backtest_ilp(start_date, end_date, token0, token1, pool_id, ddpg_agent_path, ppo_agent_path, rebalancing_frequency, agent):
        ...
        while current_date <= end_date:
            ...
            if agent == "ddpg":
                price_lower = float(ddpg_action[0][0])
                price_upper = float(ddpg_action[0][1])
            else:
                price_lower = ppo_action['price_lower']
                price_upper = ppo_action['price_upper']

            print(f"Price lower  :  {price_lower}    :  {type(price_lower)}")
            print(f"Price upper  :  {price_upper}    :  {type(price_upper)}")
    ...
    end_date = '2024-05-01'
    # ddpg_agent_name = 'ddpg_tempest_2000x50'
    ddpg_agent_path = f'model_storage/ddpg/{ddpg_agent_name}'
    ppo_agent_path = 'model_storage/ppo/ppo_tempest_2000x15'
    ...
    results_df.to_csv(f"model_output/backtest/results_{ddpg_agent_name}_2.csv")
    data_df.to_csv(f"model_output/backtest/data_{ddpg_agent_name}_2.csv")