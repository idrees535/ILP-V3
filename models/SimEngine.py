    ...
    def run(self):
        ...
        for index, row in self.df.iloc[self.current_index:end_index].iterrows():
            if row['type'] == 'mint':
                ...
                token0_amount = int(pd.to_numeric(row['amount0'], errors='coerce'))*10**10
            ...
            if row['type'] == 'swap' and int(pd.to_numeric(row['amount0'], errors='coerce', downcast='float')) < 0 :
                token0_amount = int(pd.to_numeric(row['amount0'], errors='coerce'))*10**10
                token1_amount = int(pd.to_numeric(row['amount1'], errors='coerce'))
                block_time = row['evt_block_time']
            ...
            if row['type'] == 'swap' and int(pd.to_numeric(row['amount1'], errors='coerce', downcast='float')) < 0 :
                token0_amount = int(pd.to_numeric(row['amount0'], errors='coerce'))*10**10
                token1_amount = int(pd.to_numeric(row['amount1'], errors='coerce'))
                block_time = row['evt_block_time']
