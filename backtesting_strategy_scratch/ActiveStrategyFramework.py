...
class StrategyObservation:
    def __init__(self,timepoint,
        ...
        # print(TICK_P_PRE,self.decimal_adjustment,self.price,self.tickSpacing)
...
def generate_simulation_series(simulations,strategy_in,token_0_usd_data = None):
    ...
    token_0_initial                  = simulations[0].liquidity_ranges[0]['token_0'] + simulations[0].token_0_left_over
    token_1_initial                  = simulations[0].liquidity_ranges[0]['token_1'] + simulations[0].token_1_left_over
...
def aggregate_price_data(data,frequency):
    ...
    if   frequency == '30M':
        resample_option      = '30 min'
    elif frequency == 'H':
        resample_option      = '1H'
    elif frequency == 'D':
        resample_option      = '1D'
    else:
        resample_option = frequency
            

    data_floored_min                      = data.copy()
    data_floored_min.index                = data_floored_min.index.floor('Min')   
    price_range                           = pd.DataFrame({'time_pd': pd.date_range(data_floored_min.index.min(),data_floored_min.index.max(),freq='10 min',tz='UTC')})
    price_range                           = price_range.set_index('time_pd')
    ...
    # Sampling frequncy hardcoded in price data aggregated
...
def analyze_strategy(data_usd,frequency = 'H'):
    ...
    summary_strat = {
                        ...
                        'mean_base_position'   : (data_usd['base_position_value_in_token_0']/ \
                                                  (data_usd['base_position_value_in_token_0']+data_usd['value_left_over_in_token_0'])).mean(),        
                        'median_base_position' : (data_usd['base_position_value_in_token_0']/ \
                                                  (data_usd['base_position_value_in_token_0']+data_usd['value_left_over_in_token_0'])).median(),
