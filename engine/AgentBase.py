...
class AgentBaseEvmBoth(AgentBaseAbstract):
    def __init__(self, name: str, WETH: float, USDC: float, policy_func=None):
        super().__init__(name)
        self._wallet = AgentWalletEvmBoth(WETH, USDC)  # Assuming this is defined
    ...
    def USDC(self) -> float:
        return self._wallet.USDC()
    def WETH(self) -> float:
        return self._wallet.WETH()